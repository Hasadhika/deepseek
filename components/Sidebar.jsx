...
const Sidebar = ({ expand, setExpand }) => {
  ...
  return (
    <div
      className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen overflow-hidden ${
    ...
    >
      <div>
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ?"bg-primary hover-opacity-90 rounded-2xl gap-2 p-2.5 w-max":"group relative  h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            <Image className={expand ? 'w-6s': 'w-7'} src={expand ? assets.chat_icon :assets.chat_icon_dull} alt=''/>
        ...
        <div className={`mt-8 text-white/25 text-sum ${expand ? "block ": "hidden"}`}>
      ...
      <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border boredr-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
